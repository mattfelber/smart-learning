...
import { buildTrace, stateFromTrace, type SlidingWindowVisualSpec } from '@smart-learning/shared';

interface Props {
  spec: SlidingWindowVisualSpec;
}

const SPEEDS = [1, 2, 4] as const;

export function SlidingWindowVisualizer({ spec }: Props) {
  const { nums, k } = spec;
  const trace = useMemo(() => buildTrace(nums, k), [nums, k]);
  const [displayStep, setDisplayStep] = useState(spec.stepIndex);
  ...
  const tutorStep = Math.max(0, Math.min(lastStep, spec.stepIndex));
