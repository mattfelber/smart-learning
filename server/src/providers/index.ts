...
export function createProvider(): { provider: LLMProvider; configured: boolean; error?: string } {
  if (config.LLM_PROVIDER === 'gemini') {
    if (!config.GEMINI_API_KEY) {
      return {
        provider: new GeminiProvider('placeholder', config.GEMINI_MODEL),
        configured: false,
        error:
          'Gemini API key is missing. Create a .env file from .env.example and set GEMINI_API_KEY.'
      };
    }
    return { provider: new GeminiProvider(config.GEMINI_API_KEY, config.GEMINI_MODEL), configured: true };
  }
